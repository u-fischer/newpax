...
\lstinputlisting{doc-extract.tex}
...
\item There is small typo in \pkg{pax.sty} which affects clipping, the patch shown in the listing correct this.
...
\lstinputlisting[firstline=2]{doc-pax-test.tex}

\subsection[Alternative step two: Using the \texttt{.newpax}-file with \pkg{newpax.sty}]
{\textcolor{red}{Experimental!!!}\\ Alternative step two: Using the \texttt{.newpax}-file with \pkg{newpax.sty}}


The style \pkg{newpax} is based on \texttt{pax.sty} but extended in various way. It is an experimental package which requires the experimental pdfresources code.
