...
When a PDF is included in another PDF -- may it be with \verb+\includegraphics+ or with \verb+\includepdf+ -- 
...
The \pkg{pax} package from Heiko Oberdiek offered a solution for this problem: if collected all the annotations
...
The \pkg{newpax} tries to address this problems. If offers a lua script to collect the annotations which can be run with luatex, and it works with all engines and backends. The package is based in large parts on the \pkg{pax} package: it uses its data structure and the original code to calculate the coordinates (with a few minor bug corrections), only the pdf\LaTeX{} primitives have been replaced by commands from the new \LaTeX{} PDF management in \pkg{pdfmanagement-testphase}.   
...
{Step 2: Using the \texttt{.newpax}-file with \pkg{newpax.sty}}

The style \pkg{newpax} is based on \texttt{pax.sty} but extends it in various way. 
